...
<body>
  <spring:url value="/controlli/controllofisico/nuova/salva" var="formAction" />
				...
				<div class="row">
				  ...
				  <li-lista><b>L’OP utilizza un substrato di coltivazione è idoneo al fine di garantire l’assenza di ON</b>
				                   <div class="row">													
													  ...
													  <label class="radio-inline">
															<form:radiobutton id="flControllo5x3"
																cssClass="with-gap" path="flControllo5x4"
																value="N"/>
															<span>No</span>
														</label>
...
<content tag="script"> <script>
	...
	function eliminaGenere(link) {
		   console.log('BEGIN eliminaGenere');

		   var genere;
		   var specie;	

		   $riga = $(link).closest('tr');
		   genere = $riga.find('td:nth-of-type(2)').text();
		   console.log('genere ='+genere);
		   specie = $riga.find('td:nth-of-type(3)').text();
		   console.log('specie ='+specie);

		   var messaggio;
		   if(specie != null && specie != ''){
			   messaggio = 'Si desidera eliminare il Genere : <b>'
							+ genere + '</b>, con Specie : <b>' + specie
							+ '</b>?';
		   }
		   else{
			   messaggio = 'Si desidera eliminare il Genere : <b>'
			    ...
			    if (response) {
			    	...
			    	Swal.fire({
						...
						if (result.value) {
							  ...
							  window.location.href = '<spring:url value="/controlli/controllofisico/nuova"/>';
						}
					});
			    }
			}).fail(function(jqXHR, textStatus) {
