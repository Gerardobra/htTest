...
<body>
  <spring:url value="/vivai/comunicazioni/anagrafica/modifica" var="formAction" />
  <form:form action="${formAction}" method="post"
    modelAttribute="nuovaComunicazioneForm" accept-charset="utf-8">
    
	<div class="card">
    <div class="card-header bg-primary-color white-text">
      <h4 class="card-title text-shadow uppercase-title title-padding"><b>MODIFICA COMUNICAZIONE</b></h4>
    </div>
    ...
    <div class="card-content">
      ...
      <div class="row">
        <table id="tabellaSpecie" class="data-table striped display" 
          ...
          <c:forEach var="genere" items="${nuovaComunicazioneForm.listaGeneri}">
            <tr>
              <td nowrap style="white-spaces: nowrap">
	              ...
	              <input type="hidden" name="idSpecie" value="${genere.idSpecie}" />
	              <input type="hidden" name="denomGenere" value="${genere.denomGenere}" />
              ...
              <td>
                <div class="input-field">                	                	       			 	       			  
	       			  <form:input type="text" id="numeroPiante" path="numeroPiante" value="${genere.numeroPiante}"
						class="validate" pattern="\d*" maxlength="8" required="required"
  ...
  <div class="row">
    ...
    <div class="right" style="text-align: right">
      ...
      <c:if test="${nuovaComunicazioneForm.abilitaInoltra}">
      <a href='<spring:url value="/vivai/comunicazioni/inoltra/${nuovaComunicazioneForm.idComunicazione}"/>'
        title="Inoltra"
      ...
      <c:if test="${nuovaComunicazioneForm.abilitaAnnulla}">
      <a href='<spring:url value="/vivai/comunicazioni/annulla/${nuovaComunicazioneForm.idComunicazione}"/>' 
        title="Annulla" 
      ...
      <c:if test="${nuovaComunicazioneForm.abilitaRespingi}">
      <a href='<spring:url value="/vivai/comunicazioni/respingi/${nuovaComunicazioneForm.idComunicazione}"/>' 
        title="Respingi" 
        class="btn red darken-4 waves-effect waves-light" 
        style="display:inline-block" 
      ...
      <c:if test="${nuovaComunicazioneForm.abilitaAutorizza}">
      <a href='<spring:url value="/vivai/comunicazioni/autorizza/${nuovaComunicazioneForm.idComunicazione}"/>'
        title="Autorizza"
        class="btn amber darken-1 waves-effect waves-light"
        style="display: inline-block"
  ...
  <!-- FINE MODALE -->
	...
	$(document).ready(function() {
	   ...
	   console.log('numero di generi nella pagina ='+$('#bodyTabellaSpecie input[name=denomGenere]').length);
	   ...
	   // ---------------
        ...
        $('#salvaComunicazione').click(function(event) {
        	//Si controlla che l'utente abbia inserito almeno un genere prima di effettuare il submit
        	if ($('#bodyTabellaSpecie input[name=denomGenere]').length < 1) {
        		event.preventDefault();
        		swal({
        			title : 'Aggiungere almeno un genere o specie',
        			type : 'warning',
        		});
        	}
        });
        
        <c:if test="${not empty visualizzaModalInoltra}">
        visualizzaModalInoltra = true;
        checkInoltra();
        </c:if>
	 ...
	 function getOptionCentroAziendale(centroAziendale) {
			...
			function modificaGenere(link) {
				...
				function aggiungiSpecie(){
				  ...
				  var idSpecieSel = '';
				  var descSpecieSel = '';
				  var genereSel = $('#divSpecie input[name=genere]').val();
				  ...
				  if (numSpecieTrovate > 2) {
					 ...
					 function() {
					   ...
					   console.log(' ^^^^^^ idSpecieHidden ='+ idSpecieHidden);
					   console.log(' ^^^^^^ genereHidden ='+ generiHidden);
				...
				/********** Richiamato sul pulsante 'MODIFICA' della modale (gestione inserimento e modifica)
				 */
				function salvaModificaSpecie($form) {
					...
					if ($rigaTabellaSpecie) {
						...
						denomGenereRim = $rigaTabellaSpecie.find('input[name=denomGenere]').val();
						console.log('idSpecie da rimuovere =' + idSpecieRim);
						console.log('denomGenereRim da rimuovere =' + denomGenereRim);
						...
						// Togliere l'idSpecie che si sta rimuovando anche dalla stringa idSpecieHidden, usata per filtrare la query (not in...)
						if (generiHidden) {
							console.log('generiHidden PRIMA ='+ generiHidden);
							var arrayGeneri = generiHidden.split(',');
							console.log('denomGenereRim da rimuovere ='+ denomGenereRim);
							var indiceInArray = $.inArray(denomGenereRim,arrayIdSpecie);
							console.log('indiceInArray =' + indiceInArray);
							/*  
							 *  Se viene trovato il genere nell'array, la option viene selezionata 
							 *  e il genere viene rimosso dall'array 
							 */
							if (indiceInArray >= 0) {
								arrayGeneri.splice(indiceInArray, 1);
								generiHidden = arrayGeneri.toString();
								console.log('generiHidden DOPO ='+ generiHidden);
							}
						}
					...
					if (numSpecieTrovate > 2) {
						...
						$form.find("select[name=specie] option:selected").each(function() {
								idSpecieSel = $(this).val();
								console.log('idSpecieSel ='+ idSpecieSel);
								
								// per non visualizzare le specie già selezionate
								if (idSpecieHidden) {
									idSpecieHidden += ',' + idSpecieSel;
									generiHidden +=','+genereSel;
								} else {
				...
				// IVAN 
				// PULSANTE AGGIUNGI DELLA MODALE DELLE SPECIE			
				/*function salvaModificaSpecie($form) {
					if ($form.length == 0) {
						return;
					}
				...
				function checkInoltra() {
					swal(
							{
								title : visualizzaModalInoltra ? 'I dati sono stati salvati'
										: 'Attenzione!',
								html : 'Si desidera inoltrare la comunicazione?',
								type : 'question',
								showCancelButton : true,
								confirmButtonText : 'INOLTRA',
								confirmButtonClass : 'btn amber darken-1 waves-effect waves-light',
								cancelButtonText : 'SALVA IN BOZZA',
							...
							.then(
									function(result) {
										if (result.value) {
											window.location.href = '<spring:url value="/vivai/comunicazioni/inoltra/${nuovaComunicazioneForm.idComunicazione}"/>';
										}
									});
				...
				$('#spedizioniereRuop').change(function() {
					...
					if ($(this).val().trim().length == 0 || $(this).val().trim().length > 1) {
		        	  ...
		        	  getSelectAsincrona(
		        			  'spedizioniere',
		        			  '<spring:url value="/ajax/SpedizionieriByDenomRuop" />',
		        			  params,
