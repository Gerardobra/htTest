...
<caronte:decorator titleText="Caronte - Registrazioni RUOP e Richieste Passaporto" materializeCss="materialize_autorizzazioni.css" 
  colorCode="#00acc1" gradientColor="cyan">
  <jsp:attribute name="navContent">
    <li class="tab col s2">
	    <c:choose>
	      <c:when test="${fn:contains(requestScope['javax.servlet.forward.request_uri'], '/comunicazioni/')}">
	      <a class="active" href='<spring:url value="/autorizzazioni/comunicazioni/elenco"/>' target="_self">Domande</a>
	      </c:when>
	      <c:otherwise>
	      <a href='<spring:url value="/autorizzazioni/comunicazioni/elenco"/>' target="_self">Domande</a>
	      </c:otherwise>
	    </c:choose>
    </li>
    ...
    <li class="tab col s2">
	    <c:choose>
	      ...
	      <a class="active" href='<spring:url value="/autorizzazioni/report/elenco"/>' target="_self">Report</a>
	      </c:when>
	      <c:otherwise>
	      <a href='<spring:url value="/autorizzazioni/report/elenco"/>' target="_self">Report</a>
	      </c:otherwise>
	    </c:choose>
    </li>
    </sec:authorize>
