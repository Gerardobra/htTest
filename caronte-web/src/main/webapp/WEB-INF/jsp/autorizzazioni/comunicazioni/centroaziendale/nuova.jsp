...
<body>
	<spring:url value="/autorizzazioni/comunicazioni/centroaziendale/nuova/salva" var="formAction" />
			...
			<sec:authorize access="hasRole('SUPERUSER')" var="isSuperUser" />
  			...
  			<div class="card-content" style="padding-top:5px;">
  				...
  				<!--   <div class="input-field col s12 m8 l4">
  				    <label for="numCentriAz">Numero centri aziendali</label>
					<form:input type="text" id="numCentriAz"
							path="numCentriAz" required="required" />
					<form:errors path="numCentriAz" cssClass="error" />					
				  </div>-->
				  <!-- div class="col l8">
							<label>
								<form:checkbox id="idCentroAzNoSedeLeg" value="1" path="idCentroAzNoSedeLeg" />
								<span>Esistono centri aziendali in luogo diverso dalla sede legale</span>
							</label>
				  </div>-->
				  <!-- /div>
				  </div>
	...
	*/		
     ...
     // ********** Gestione AGGIUNGI TIPOLOGIA PRODUTTIVA
        ...
        $('#provinciaSitoProd').change(function() {
			// ------ Popolamento combo comune
			console.log('Popolamento combo comune modale sito produzione');
			var params = {
				"idProvincia" : $(this).val()			
			}
        ...
        function eliminaSitoProd(link) {		  
		  ...
		  if(sitoProduz !='' &&  ubicazione != ''){
			  messaggio ='Si desidera eliminare il sito produzione : <b>' + sitoProduz 
		    				+ '</b>, con indirizzo : <b>' + ubicazione +'</b>?'
		  }
		  else{
			  if(sitoProduz =='' &&  ubicazione == ''){
				  ...
				  if(sitoProduz != ''){
					  messaggio = 'Si desidera eliminare il sito produzione : <b>' + sitoProduz + '</b>?'; 
				  }
				  else if(ubicazione != ''){
					  messaggio = 'Si desidera eliminare il sito produzione con indirizzo : <b>'+ubicazione+'</b>?';
				  }
			  }
          }
		   swal({
     ...
     function aggiungiCentroAziendale(){
       ...
       var denominazCentroAz = $("#denominazCentroAz").val();
       console.log('denominazCentroAz ='+denominazCentroAz);
	   ...
	   console.log('indirizzoCentroAz senza replace ='+indirizzoCentroAz);
	   //faccio una raplace nel caso ci fossero slash nel civico
       var indirizzoCaEscape = indirizzoCentroAz.replace(/\//g, "-");
       console.log('indirizzoCentroAz con replace ='+indirizzoCaEscape);
       ...
       if(  (idProvCentroAz == null || idProvCentroAz == '') ||
    	 ...
    	 /*  console.log('Effettuo submit centro aziendale');
    	   ...
    	   $.ajax({
			    // Ricerca del centro aziendale su car_t_centro_aziendale		
			    //encodeURIComponent(indirizzoCentroAz), non funziona correttamente su macchiene unix(problema il charset)
				url : '<spring:url value="/autorizzazioni/comunicazioni/checkIfCentroAzExists_"/>'+idCentroAzSelezionato+'_'+ indirizzoCaEscape + '_'+idComuneCentroAz,
				success : function(response) {	
	   				if (response.indexOf("OK") == -1) {
     		  			...
     		  			return; 
  	   					...
  	   					console.log("Centro aziendale OK, nuovo"); 
  		   				console.log('Effettuo submit centro aziendale');
				...
				error: function(XMLHttpRequest, textStatus, errorThrown) { 
	                 console.log("Status checkIfCentroAzExists: " + textStatus); 
	                 console.log("Error checkIfCentroAzExists: " + errorThrown); 
               }    
   	   		});
       } 
     }
  ...
  // --- CLICK SUL PULANTE SALVA
   ...
   function eliminaCentroAz(link) {		  
	...
	swal({
				title : 'Attenzione!',
			    html : 'Si desidera eliminare il Centro aziendale : <b>' + centroAz +'</b> dalla domanda?',
			    type : 'warning',
	   			showCancelButton : true,
				confirmButtonText : 'Sì',
				cancelButtonText : 'No',
