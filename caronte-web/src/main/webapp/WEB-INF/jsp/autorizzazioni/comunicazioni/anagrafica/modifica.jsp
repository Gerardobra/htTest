...
<script type="text/javascript">
	/*Usata per determinare se c'è la necessità di disabilitare tutti i campi sulla pagina*/
		var needToDisableFields = false;
	</script>
</head>
<body>
	<spring:url value="/autorizzazioni/comunicazioni/anagrafica/modifica"
		var="formAction" />
		...
		<div class="card">
			...
			<div class="card-content" style="padding-top: 5px;">
						...
						<ul class="tabs">
							...
							<c:if test="${not empty nuovaDomandaForm.idDomanda}">
								...
								<c:if
									test="${idTipoDomandaCancellazioneRUOP != nuovaDomandaForm.idTipoComunicazione}">
									<c:if
										test="${isSuperUser and statoInBozza != nuovaDomandaForm.idStatoComunicazione}">
										<li class="tab"><a
											href="<spring:url value="/autorizzazioni/comunicazioni/gestione/modifica" />"
											target="_self">Gestione</a></li>
									</c:if>
								</c:if>
							</c:if>
							...
							<c:choose>
								...
								<c:otherwise>
									<div class="input-field col s6 m4 l3">
										<form:input type="text" id="cognome" path="cognome"
											cssClass="validate campiUt" required="required"
											maxlength="50" disabled="true" data-todisable="true" />
										<form:errors path="cognome" cssClass="error" />
										<label for="cognome">Cognome *</label>
									</div>
									<div class="input-field col s6 m4 l3">
				...
				<div class="row">
					...
					<!--<div class="input-field col s6 m4 l3">
						<form:input type="text" id="denomComuneEstNascita"
							path="denomComuneEstNascita" cssClass="validate campiUt"
							required="required" maxlength="100" disabled="true"/>
						<form:errors path="denomComuneEstNascita" cssClass="error" />
				...
				<div class="right" style="text-align: right">
					...
					<c:if test="${nuovaDomandaForm.abilitaCreaNuovaDomandaFlusso}">
						<a
							...
							style="display: inline-block">NUOVA DOMANDA</a>
					</c:if>
					<!-------->
					<c:if
					...
					<button id="salvaDomanda" type="submit"
						class="btn confirm waves-effect waves-light"
	...
	<c:if
		test="${nuovaDomandaForm.idTipoComunicazione == idTipoDomandaVariazioneRUOP 
        						&& (nuovaDomandaForm.idStatoComunicazione == statoInBozza) }">
		<script>
										needToDisableFields = true;
        								//disableFields();
        							</script>
	</c:if>
	   ...
	   // *** Checkbox 'Residenza in Stato estero 'residenzaEstera'
		$('#residenzaEstera').change(function() {
			...
			function checkResidenzaEstera(){
				...
				M.updateTextFields();
				if(needToDisableFields){
					console.log("check residenza estera");
					//disableFields();
				}
			...
			// Se viene checkato 'Nascita in Stato estero' devono essere disbilitati e non visualizzati i campi : 'Provincia nascita e Comune nascita', mentre deve essere abilitato e visualizzato il campo 'Comune estero di nascita'
			...
			function checkNascitaEstera() {	
				...
				M.updateTextFields();
				if(needToDisableFields){
					console.log("check nascita estera");
					//disableFields();
				}
