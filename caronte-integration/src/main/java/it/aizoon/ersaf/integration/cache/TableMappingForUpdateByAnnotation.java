package it.aizoon.ersaf.integration.cache;
...
import it.aizoon.ersaf.integration.IPersistent;
import it.aizoon.ersaf.integration.annotation.DBTable;
import it.aizoon.ersaf.integration.annotation.DBUpdateColumn;
...
public class TableMappingForUpdateByAnnotation {
  ...
  public TableMappingForUpdateByAnnotation(Class<?> classObj, DBTable dbTable) throws DatabaseAutomationException {
    tableName = dbTable.value();
    objectClassName = classObj.getName();
    parseClass(classObj);
  }

  public TableMappingForUpdateByAnnotation(Class<?> classObj) throws DatabaseAutomationException {
    this(classObj, classObj.getAnnotation(DBTable.class));
  }

  protected void parseClass(Class<?> classObj) throws DatabaseAutomationException {
    Class<?> baseClass = classObj;

    while (classObj != null && classObj != Object.class && !classObj.getName().contains("java.lang.Object")) {
      for (Field f : classObj.getDeclaredFields()) {
        DBUpdateColumn dbUpdateColumnAnnotation = (DBUpdateColumn)f.getAnnotation(DBUpdateColumn.class);

        if (dbUpdateColumnAnnotation != null) {
          processDBUpdateColumnAnnotation(f, dbUpdateColumnAnnotation, baseClass);
        }
      }
  ...
  public MapSqlParameterSource getSqlParameterSource(IPersistent updateObject, boolean update)
      throws DatabaseAutomationException {
    MapSqlParameterSource source = new MapSqlParameterSource();
    for (Column column : columns) {
      String defaultValue = update ? column.getDefaultUpdateValue() : column.getDefaultInsertValue();
      ...
      if (defaultValue == null || defaultValue.length() == 0) {
        ...
        try {
          ...
          throw new DatabaseAutomationException(
              CaronteUtils.getMessage("error.query.class_retrieval", objectClassName), e);
        }
  ...
  protected void processDBUpdateColumnAnnotation(Field f, DBUpdateColumn annotation, Class<?> baseClass)
      throws DatabaseAutomationException {
    try {
      Method getter = baseClass.getMethod(getGetter(f.getName()));
      Column column = new Column(annotation.value(), annotation.whereClause(), annotation.defaultInsertValue(),
          annotation.defaultUpdateValue(), annotation.type(), getter);
      columns.add(column);
    }catch (Exception e) {
      throw new DatabaseAutomationException(
          CaronteUtils.getMessage("error.query.update.class_analysis", baseClass.getName()));
    }
  }
  ...
  protected String createSQLForUpdate() throws DatabaseAutomationException {
    ...
    if (sbWhere.length() == 0) {
      throw new DatabaseAutomationException(
          CaronteUtils.getMessage("error.query.no_where_clause", "UPDATE", objectClassName));
    }
    sbUpdate.append("\nWHERE\n");
  ...
  public String createSQLForInsert() throws DatabaseAutomationException {
    ...
    if (sbValues.length() == 0) {
      throw new DatabaseAutomationException(
          CaronteUtils.getMessage("error.query.no_where_clause", "INSERT", objectClassName));
    }
